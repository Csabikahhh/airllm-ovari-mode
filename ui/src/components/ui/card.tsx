...
function Card({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      data-slot="card"
      className={cn("bg-card text-card-foreground flex min-w-0 flex-col rounded-lg border shadow-sm", className)}
      {...props}
    />
  )
}
...
function CardHeader({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn("grid auto-rows-min grid-rows-[auto_auto] items-start gap-1.5 px-4 pt-4 sm:px-5 sm:pt-5", className)}
      {...props}
    />
  )
}
